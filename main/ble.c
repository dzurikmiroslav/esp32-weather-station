...
static const uint16_t GATTS_CHAR_UUID_EXT_BAT_LVL  = 0x3A19; // 0x2A19 reserved for GATTS_CHAR_UUID_INT_BAT_LVL
...
static uint8_t ext_bat_lvl_val = 0;
static uint8_t ext_bat_lvl_ccc[2] = {0x00, 0x00};
...
static const esp_gatts_attr_db_t gatt_bat_serv_db[BAT_SERV_IDX_NB] = {
    ...
    [BAT_SERV_IDX_CHAR_BAT_LVL] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&char_prop_read_notify}},
    [BAT_SERV_IDX_CHAR_VAL_BAT_LVL] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_EXT_BAT_LVL, PERM_READ, sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&ext_bat_lvl_val}},
    [BAT_SERV_IDX_CHAR_CFG_BAT_LVL] =
        {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_client_config_uuid, PERM_READ_WRITE, 2 * sizeof(uint8_t), 2 * sizeof(uint8_t), (uint8_t *)ext_bat_lvl_ccc}},
...
void ble_set_ext(float humidity, float temperature, float pressure, uint8_t battery)
{
    ...
    ext_bat_lvl_val = battery;

    esp_ble_gatts_set_attr_value(env_sens_handle_table[ENV_SENS_IDX_CHAR_VAL_EXT_HUM], sizeof(uint16_t),
            (uint8_t *) &ext_hum_val);
    esp_ble_gatts_set_attr_value(env_sens_handle_table[ENV_SENS_IDX_CHAR_VAL_EXT_TEMP], sizeof(int16_t),
            (uint8_t *) &ext_temp_val);
    esp_ble_gatts_set_attr_value(env_sens_handle_table[ENV_SENS_IDX_CHAR_VAL_EXT_PRESS], sizeof(uint32_t),
            (uint8_t *) &ext_press_val);
    esp_ble_gatts_set_attr_value(bat_serv_handle_table[BAT_SERV_IDX_CHAR_VAL_BAT_LVL], sizeof(uint8_t),
            (uint8_t *) &ext_bat_lvl_val);
    if (ble_has_connection) {
        esp_ble_gatts_send_indicate(ble_gatts_if, ble_connection_id,
                env_sens_handle_table[ENV_SENS_IDX_CHAR_VAL_EXT_HUM], sizeof(uint16_t), (uint8_t *) &ext_hum_val,
                false);
        esp_ble_gatts_send_indicate(ble_gatts_if, ble_connection_id,
                env_sens_handle_table[ENV_SENS_IDX_CHAR_VAL_EXT_TEMP], sizeof(int16_t), (uint8_t *) &ext_temp_val,
                false);
        esp_ble_gatts_send_indicate(ble_gatts_if, ble_connection_id,
                env_sens_handle_table[ENV_SENS_IDX_CHAR_VAL_EXT_PRESS], sizeof(uint32_t), (uint8_t *) &ext_press_val,
                false);
        esp_ble_gatts_send_indicate(ble_gatts_if, ble_connection_id,
                bat_serv_handle_table[BAT_SERV_IDX_CHAR_VAL_BAT_LVL], sizeof(uint8_t), (uint8_t *) &ext_bat_lvl_val,
                false);
    }
}
