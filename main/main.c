...
static void ext_data_push_history(ext_data_t *data)
{
	...
	if (now >= (data->history.write_time + HISTORY_PERIOD / portTICK_PERIOD_MS))
	{
		...
		data->history.humidity[HISTORY_SIZE - 1] = data->history.avg_humidity
				/ data->history.avg_counter;
		data->history.pressure[HISTORY_SIZE - 1] = data->history.avg_pressure
				/ data->history.avg_counter;
		data->history.temperature[HISTORY_SIZE - 1] =
				data->history.avg_temperature / data->history.avg_counter;
...
static void int_data_push_history(int_data_t *data)
{
	...
	if (now >= (data->history.write_time + HISTORY_PERIOD / portTICK_PERIOD_MS))
	{
		...
		data->history.temperature[HISTORY_SIZE - 1] =
				data->history.avg_temperature / data->history.avg_counter;
		data->history.co2[HISTORY_SIZE - 1] = data->history.avg_co2
				/ data->history.avg_counter;
		data->history.tvoc[HISTORY_SIZE - 1] = data->history.avg_tvoc
				/ data->history.avg_counter;
	...
	while (true)
	{
		...
		if (queue_member == lcd_print_sensor_queue)
		{
			...
			bool change_sensor =
					now
							>= (sensor_change_time
									+ SCREN_TIMEOUT / portTICK_PERIOD_MS);
			bool change_screen = false;
			bool new_data = false;

			if (now
					>= ext_data.write_time
							+ EXT_SENSOR_TIMEUT / portTICK_PERIOD_MS)
			{
		...
		if (refresh)
		{
			...
			case LCD_SCREEN_TYPE_INT_MAIN:
				display_print_int_value(int_data.temperature, int_data.humidity,
						int_data.co2, int_data.tvoc, ble_connection);
				break;
			case LCD_SCREEN_TYPE_INT_HUMIDITY_MIN_MAX:
				display_print_min_max(SENSOR_ID_INTERNAL,
						DISPLAY_VALUE_HUMIDITY, int_data.history.humidity);
				break;
			case LCD_SCREEN_TYPE_INT_TEMPERATURE_MIN_MAX:
				display_print_min_max(SENSOR_ID_INTERNAL,
						DISPLAY_VALUE_TEMPERATURE,
						int_data.history.temperature);
				break;
			case LCD_SCREEN_TYPE_INT_CO2_MIN_MAX:
				display_print_min_max(SENSOR_ID_INTERNAL, DISPLAY_VALUE_CO2,
						int_data.history.co2);
				break;
			case LCD_SCREEN_TYPE_INT_TVOC_MIN_MAX:
				display_print_min_max(SENSOR_ID_INTERNAL, DISPLAY_VALUE_TVOC,
						int_data.history.tvoc);
				break;
			case LCD_SCREEN_TYPE_INT_HUMIDITY_GRAPH:
				display_print_graph(SENSOR_ID_INTERNAL, DISPLAY_VALUE_HUMIDITY,
						int_data.history.humidity);
				break;
			case LCD_SCREEN_TYPE_INT_TEMPERATURE_GRAPH:
				display_print_graph(SENSOR_ID_INTERNAL,
						DISPLAY_VALUE_TEMPERATURE,
						int_data.history.temperature);
				break;
			case LCD_SCREEN_TYPE_INT_CO2_GRAPH:
				display_print_graph(SENSOR_ID_INTERNAL, DISPLAY_VALUE_CO2,
						int_data.history.co2);
				break;
			case LCD_SCREEN_TYPE_INT_TVOC_GRAPH:
				display_print_graph(SENSOR_ID_INTERNAL, DISPLAY_VALUE_TVOC,
						int_data.history.tvoc);
				break;
			case LCD_SCREEN_TYPE_EXT_MAIN:
				display_print_ext_value(ext_data.pressure, ext_data.temperature,
						ext_data.humidity, ble_connection);
				break;
			case LCD_SCREEN_TYPE_EXT_HUMIDITY_MIN_MAX:
				display_print_min_max(SENSOR_ID_EXTERNAL,
						DISPLAY_VALUE_HUMIDITY, ext_data.history.humidity);
				break;
			case LCD_SCREEN_TYPE_EXT_PRESSURE_MIN_MAX:
				display_print_min_max(SENSOR_ID_EXTERNAL,
						DISPLAY_VALUE_PRESSURE, ext_data.history.pressure);
				break;
			case LCD_SCREEN_TYPE_EXT_TEMPERATURE_MIN_MAX:
				display_print_min_max(SENSOR_ID_EXTERNAL,
						DISPLAY_VALUE_TEMPERATURE,
						ext_data.history.temperature);
				break;
			case LCD_SCREEN_TYPE_EXT_HUMIDITY_GRAPH:
				display_print_graph(SENSOR_ID_EXTERNAL, DISPLAY_VALUE_HUMIDITY,
						ext_data.history.humidity);
				break;
			case LCD_SCREEN_TYPE_EXT_PRESSURE_GRAPH:
				display_print_graph(SENSOR_ID_EXTERNAL, DISPLAY_VALUE_PRESSURE,
						ext_data.history.pressure);
				break;
			case LCD_SCREEN_TYPE_EXT_TEMPERATURE_GRAPH:
				display_print_graph(SENSOR_ID_EXTERNAL,
						DISPLAY_VALUE_TEMPERATURE,
						ext_data.history.temperature);
				break;
			}
...
static void bme_read_task_func(void *param)
{
	...
	while (true)
	{
		...
		ble_set_int_humidity_temperature(int_data.humidity,
				int_data.temperature);
...
static void IRAM_ATTR button_isr_handler(void *arg)
{
   ...
   else
   {
	...
	{
		if (now
				>= handler_data->click_time
						+ BUTTON_MIN_TRESHOLD / portTICK_PERIOD_MS)
		{
			lcd_evt_t evt =
					handler_data->button == BUTTON_0 ?
							LCD_EVT_CHANGE_SENSOR : LCD_EVT_CHANGE_SCREEN;
		...
		int len)
{
	...
	ESP_LOGI(LOG_TAG, "Received sensor data, battery capacity %d %%", sensor_data->battery);
	...
	ble_set_ext_humidity_temperature_pressure(ext_data.humidity,
			ext_data.temperature, ext_data.pressure);
...
static void espnow_init()
{
	...
	{
		sensor_peer.peer_addr[i] = (uint8_t) strtol(mac + (3 * i),
				(char **) NULL, 16);
	}
	ESP_LOGI(LOG_TAG, "Sensor MAC %02x:%02x:%02x:%02x:%02x:%02x",
			sensor_peer.peer_addr[0], sensor_peer.peer_addr[1],
			sensor_peer.peer_addr[2], sensor_peer.peer_addr[3],
			sensor_peer.peer_addr[4], sensor_peer.peer_addr[5]);
...
static void gpio_interupt_init()
{
	...
	ESP_ERROR_CHECK(
			gpio_isr_handler_add(BUTTON_0, button_isr_handler, &button_handler_data[0]));
	ESP_ERROR_CHECK(
			gpio_isr_handler_add(BUTTON_1, button_isr_handler, &button_handler_data[1]));
...
void app_main()
{
   ...
   ble_reset_mutex = xSemaphoreCreateMutex();
	...
	xTaskCreate(ccs_read_task_func, "ccs_read_task", 4 * 1024, NULL, 5,
			&ccs_read_task);
	xTaskCreate(lcd_print_sensor_task_func, "lcd_print_sensor_task", 4 * 1024,
	NULL, 5, &lcd_print_sensor_task);
#if ENABLE_BLE_SECURITY
