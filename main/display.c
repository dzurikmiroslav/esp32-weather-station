...
const uint8_t degree_grade_glyph[] = { 0x00, 0x06, 0x09, 0x09, 0x06 };

const uint8_t tilde_glyph[] = { 0x08, 0x04, 0x08, 0x10, 0x08 };

const uint8_t bluetooth_glyph[] = { 0x22, 0x14, 0x7F, 0x49, 0x36 };

const uint8_t ind_icon[] = { 0xFF, 0xFF, 0xFF, 0x81, 0xFF, 0xFF, 0xFF, 0xFF,
		0x81, 0xFB, 0xF7, 0xEF, 0x81, 0xFF, 0xFF, 0x81, 0xBD, 0xBD, 0xBD, 0xC3,
		0xFF, 0x00, 0x3C, 0x42, 0x42, 0x42, 0x3C, 0x00, 0x00, 0x3E, 0x40, 0x40,
		0x40, 0x3E, 0x00, 0x00, 0x02, 0x02, 0x7E, 0x02, 0x02, 0x00 };

const uint8_t out_icon[] = { 0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00,
		0x7E, 0x04, 0x08, 0x10, 0x7E, 0x00, 0x00, 0x7E, 0x42, 0x42, 0x42, 0x3C,
		0x00, 0xFF, 0xC3, 0xBD, 0xBD, 0xBD, 0xC3, 0xFF, 0xFF, 0xC1, 0xBF, 0xBF,
		0xBF, 0xC1, 0xFF, 0xFF, 0xFD, 0xFD, 0x81, 0xFD, 0xFD, 0xFF };

pcd8544_spi_pin_config_t spi_pin_config = { .mosi_io_num = SPI_DIN,
		.sclk_io_num = SPI_CLK, .spics_io_num = SPI_CE, };

pcd8544_control_pin_config_t control_pin_config = { .reset_io_num = SPI_RST,
		.dc_io_num = SPI_DC, .backlight_io_num = PCD8544_BL, };

pcd8544_config_t config = { .spi_host = HSPI_HOST, .is_backlight_common_anode =
		false, };
...
void display_print_int_value(float temperature, float humidity, uint16_t co2,
		uint16_t tvoc, display_icon_t icon) {
	pcd8544_set_pos(0, 0);
...
void display_print_graph(sensor_id_t sensor_id, display_value_t value,
	...
	min_bound -=
			min_bound > 0 ?
					min_bound % unit_size :
					unit_size - abs(min_bound) % unit_size;
	max_bound +=
			max_bound < 0 ?
					abs(max_bound) % unit_size :
					unit_size - max_bound % unit_size;
	...
	pcd8544_set_pos(0, 0);
	pcd8544_draw_bitmap(sensor_id == SENSOR_ID_INTERNAL ? ind_icon : out_icon,
			7, 6, false);
	pcd8544_finalize_frame_buf();
...
void display_print_min_max(sensor_id_t sensor_id, display_value_t value,
		float data[HISTORY_SIZE]) {
	pcd8544_set_pos(0, 0);
	pcd8544_draw_bitmap(sensor_id == SENSOR_ID_INTERNAL ? ind_icon : out_icon,
			7, 6, false);
